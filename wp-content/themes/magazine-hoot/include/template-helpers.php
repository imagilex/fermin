 ...
 * @return void
 */
if ( !function_exists( 'maghoot_header_branding' ) ):
function maghoot_header_branding() {
	?>
	<div <?php hybridextend_attr( 'branding' ); ?>>
		<div id="site-logo" class="<?php
			echo 'site-logo-' . esc_attr( maghoot_get_mod( 'logo' ) );
			if ( maghoot_get_mod('logo_background_type') == 'accent' )
				echo ' accent-typo with-background';
			elseif ( maghoot_get_mod('logo_background_type') == 'background' )
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_logo' ) ):
function maghoot_logo() {
	$display = '';
	$maghoot_logo = maghoot_get_mod( 'logo' );

	if ( 'text' == $maghoot_logo || 'custom' == $maghoot_logo ) {

		$tag = ( is_front_page() ) ? 'h1' : 'div';
		$title_icon = maghoot_get_mod( 'site_title_icon', NULL );

		$class = ( $title_icon ) ? ' site-logo-with-icon' : '';
		if ( !function_exists( 'maghoot_theme_premium' ) ) {
			$class .= ( 'text' == $maghoot_logo ) ? ' site-logo-text-' . maghoot_get_mod( 'logo_size' ) : '';
		}
		$class = ( empty( $class ) ) ? '' : ' class="' . $class . '"';

		$display .= '<div id="site-logo-' . esc_attr( $maghoot_logo ) . '"' . $class . '>';
			...
			if ( 'text' == $maghoot_logo ) {
				$title = get_bloginfo( 'name' );
			} elseif ( 'custom' == $maghoot_logo ) {
				$title = maghoot_get_custom_text_logo();
			}

			$display .= apply_filters( 'maghoot_site_title', $title );
			$display .= "</a></{$tag}>";

			if ( maghoot_get_mod( 'show_tagline' ) )
				$display .= hybrid_get_site_description();

		$display .= '</div><!--logotext-->';

	} elseif ( 'mixed' == $maghoot_logo || 'mixedcustom' == $maghoot_logo ) {

		$tag = ( is_front_page() ) ? 'h1' : 'div';
		$logo_image = ( function_exists( 'get_custom_logo' ) ) ?
						get_custom_logo() :
						'<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">' .
							'<img src="' . esc_url( maghoot_get_mod( 'logo_image' ) ) . '" />' .
						'</a>';
		$has_logo = ( function_exists( 'get_custom_logo' ) ) ? has_custom_logo() : maghoot_get_mod( 'logo_image' );

		$class = ( $has_logo ) ? 'site-logo-with-image' : '';
		if ( !function_exists( 'maghoot_theme_premium' ) ) {
			$class .= ( 'mixed' == $maghoot_logo ) ? ' site-logo-text-' . maghoot_get_mod( 'logo_size' ) : '';
		}
		$class = ( empty( $class ) ) ? '' : ' class="' . $class . '"';
		...
		$display .= '<div id="site-logo-' . esc_attr( $maghoot_logo ) . '"' . $class . '>';
			...
			$title = '';
			if ( 'mixed' == $maghoot_logo ) {
				$title = get_bloginfo( 'name' );
			} elseif ( 'mixedcustom' == $maghoot_logo ) {
				$title = maghoot_get_custom_text_logo();
			}
	...
	} elseif ( 'image' == $maghoot_logo ) {
		$display .= maghoot_get_logo_image();
	}

	echo apply_filters( 'maghoot_display_logo', $display, $maghoot_logo );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'maghoot_get_custom_text_logo' ) ):
function maghoot_get_custom_text_logo() {
	$title = '';
	$logo_custom = apply_filters( 'maghoot_logo_custom_text', hybridextend_sortlist( maghoot_get_mod( 'logo_custom' ) ) );
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_get_logo_image' ) ):
function maghoot_get_logo_image() {
	$tag = ( is_front_page() ) ? 'h1' : 'div';
	$logo_image = ( function_exists( 'get_custom_logo' ) ) ?
					get_custom_logo() :
					'<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">' .
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_header_aside' ) ):
function maghoot_header_aside() {
	$area = esc_attr( maghoot_get_mod( 'primary_menuarea' ) );
	if ( $area == 'none' )
		return;
	...
	?><div <?php hybridextend_attr( 'header-aside', '', "header-aside-{$area}" ); ?>><?php
		...
		if ( $area == 'custom' ):
			echo wp_kses_post( maghoot_get_mod( 'primary_menuarea_custom' ) );
		endif;
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_secondary_menu' ) ):
function maghoot_secondary_menu( $location ) {
	$menu_location = maghoot_get_mod( 'secondary_menu_location' );
	...
	if ( $location == $menu_location ) {
		?>
		<div <?php hybridextend_attr( 'header-part', 'supplementary', $menu_side_class ); ?>>
			<div class="hgrid">
				<div class="hgrid-span-12">
					<?php
					if ( $menu_side )
						echo '<div ' . hybridextend_get_attr( 'navarea-table', '', 'table' ) . '><div ' . hybridextend_get_attr( 'menu-nav-box', 'menu-side', 'table-cell-mid' ) . '>';
					else
					...
					echo '</div>';
					if ( $menu_side ) {
						echo '<div ' . hybridextend_get_attr( 'menu-side-box', '', 'table-cell-mid inline_nav social-icons-invert' ) . '>';
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_update_browser' ) ):
function maghoot_update_browser() {
	$notice = '<!--[if lte IE 9]><p class="chromeframe">' .
			  sprintf( __( 'You are using an outdated browser (IE 8 or before). For a better user experience, we recommend %1$1supgrading your browser today%2$2s or %3$3sinstalling Google Chrome Frame%4$4s', 'magazine-hoot' ), '<a href="http://browsehappy.com/">', '</a>', '<a href="http://www.google.com/chromeframe/?redirect=true">', '</a>' ) .
			  '</p><![endif]-->';
	echo apply_filters( 'maghoot_update_browser_notice', $notice );
}
endif;
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_remove_readmore_link' ) ):
function maghoot_remove_readmore_link() {
	add_filter( 'maghoot_readmore', 'maghoot_readmore_empty_string' );
}
endif;
if ( !function_exists( 'maghoot_readmore_empty_string' ) ):
function maghoot_readmore_empty_string() {
	return '';
}
endif;
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_reinstate_readmore_link' ) ):
function maghoot_reinstate_readmore_link() {
	remove_filter( 'maghoot_readmore', 'maghoot_readmore_empty_string' );
}
endif;
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_display_loop_title_content' ) ):
function maghoot_display_loop_title_content( $location = 'pre', $context = '' ) {

	$pre_title_content_post = apply_filters( 'maghoot_loop_meta_pre_title_content_post', '', $location, $context );
	if ( ( $location == 'pre' && !$pre_title_content_post ) ||
		 ( $location == 'post' && $pre_title_content_post ) ) : 

		$pre_title_content = apply_filters( 'maghoot_loop_meta_pre_title_content', '', $location, $context );
		if ( !empty( $pre_title_content ) ) :

			$pre_title_content_stretch =  apply_filters( 'maghoot_loop_meta_pre_title_content_stretch', '', $location, $context ); ?>
 ...
 * @return array|bool
 */
if ( !function_exists( 'maghoot_meta_info_display' ) ):
function maghoot_meta_info_display( $args = '', $context = '', $bool = false ) {
	...
	$display = apply_filters( 'maghoot_meta_info_blocks_display', $display, $context );
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_meta_info_blocks' ) ):
function maghoot_meta_info_blocks( $args = '', $context = '' ) {

	if ( !is_array( $args ) )
		$args = array_map( 'trim', explode( ',', $args ) );

	$display = maghoot_meta_info_display( $args, $context );
	...
	<div class="entry-byline">
		...
		if ( !empty( $display['author'] ) ) :
			$blocks['author']['label'] = __( 'By:', 'magazine-hoot' );
			ob_start();
		...
		if ( !empty( $display['date'] ) ) :
			$blocks['date']['label'] = __( 'On:', 'magazine-hoot' );
		...
		if ( !empty( $display['cats'] ) ) :
			...
			if ( !empty( $category_list ) ) :
				$blocks['cats']['label'] = __( 'In:', 'magazine-hoot' );
				$blocks['cats']['content'] = $category_list;
			endif;
		endif;

		if ( !empty( $display['tags'] ) && get_the_tags() ) :
			$blocks['tags']['label'] = __( 'Tagged:', 'magazine-hoot' );
			$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'magazine-hoot' ) : get_the_tag_list( '', ', ', '' );
		endif;
		...
		if ( $edit_link = get_edit_post_link() ) :
			...
			$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'magazine-hoot' ) . '</a>';
		endif;

		$blocks = apply_filters( 'maghoot_meta_info_blocks', $blocks, $context, $display );
		...
		if ( !empty( $display['publisher'] ) ) {
			...
			echo apply_filters( 'maghoot_entry_byline_publisher', $microdatapublisher );
		}
		?>
 ...
 * @return array
 */
if ( !function_exists( 'maghoot_meta_info_blocks_forcedisplay' ) ):
function maghoot_meta_info_blocks_forcedisplay( $display, $context ) {
...
add_filter( 'maghoot_meta_info_blocks_display', 'maghoot_meta_info_blocks_forcedisplay', 5, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_post_thumbnail' ) ):
function maghoot_post_thumbnail( $classes = '', $size = '', $microdata = false, $link = '', $crop = NULL ) {
	...
	$thumbnail_size = maghoot_thumbnail_size( $size, $crop );
	...
	if ( $microdata ) {
		...
		if ( !empty( $iid ) ) {
			...
			if ( !empty( $isrc[0] ) ) {
				$iwidth = ( empty( $isrc[1] ) ) ? '' : $isrc[1];
				$iheight = ( empty( $isrc[2] ) ) ? '' : $isrc[2];
				echo apply_filters( 'maghoot_post_thumbnail_microdata',
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_thumbnail_size' ) ):
function maghoot_thumbnail_size( $size = '', $crop = NULL ) {
	...
	if ( !empty( $size ) ) {
		...
		$size = 'span-' . maghoot_main_layout( 'content' );
		$thumbnail_size = hybridextend_get_image_size_name( $size, $crop );
	}

	/* Let child themes filter the size name */
	$thumbnail_size = apply_filters( 'maghoot_post_thumbnail' , $thumbnail_size, $size, $crop );
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_widget_border_class' ) ):
function maghoot_widget_border_class( $val, $index=0, $prefix='' ) {
 ...
 * @return string
 */
if ( !function_exists( 'maghoot_widget_margin_style' ) ):
function maghoot_widget_margin_style( $mt='', $mb='' ) {
	$return = '';
	if ( $mt===0 || $mt==='0' ) {
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_widgetparam_hook' ) ):
function maghoot_widgetparam_hook() {
	if ( ! is_admin() )
		add_filter( 'dynamic_sidebar_params', 'maghoot_modify_widgetparams' );
}
endif;
add_action( 'wp_loaded', 'maghoot_widgetparam_hook' );
 ...
 * @return array
 */
if ( !function_exists( 'maghoot_modify_widgetparams' ) ):
function maghoot_modify_widgetparams( $params ) {
	global $wp_registered_widgets;
	if ( !isset( $params[0] ) || !isset( $params[0]['widget_id'] ) )
		return $params;
	$widget_id = $params[0]['widget_id']; // Current widget id Eg: maghoot-content-blocks-widget-16
	...
	$widget_opt = get_option( $widget_obj['callback'][0]->option_name ); // all instance values of a particular widget (example: option_name = 'widget_maghoot-content-blocks-widget' )
	...
	if ( !empty( $instance['customcss'] ) ) {
		...
		$string = maghoot_widget_margin_style( $mt, $mb ) . $string;
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_footer_structure' ) ):
function maghoot_footer_structure() {
	$footers = maghoot_get_mod( 'footer' );
	$structure = array(
 ...
 * @return int
 */
function maghoot_get_footer_columns() {
	$footers = maghoot_get_mod( 'footer' );
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_col_width_to_span' ) ):
function maghoot_col_width_to_span( $col_width ) {
	$return = array();
	switch( $col_width ):
		case '100':
...
/**
 * Wrapper function for maghoot_main_layout() to get the class names for current context.
 ...
 * @return string
 */
if ( !function_exists( 'maghoot_main_layout_class' ) ):
function maghoot_main_layout_class( $context ) {
	return maghoot_main_layout( $context, 'class' );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'maghoot_main_layout' ) ):
function maghoot_main_layout( $context, $return = 'size' ) {
	...
	$span_content = $maghoot_theme->currentlayout['content'];
	$layout_class = ' layout-' . $maghoot_theme->currentlayout['layout'];
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_set_main_layout' ) ):
function maghoot_set_main_layout() {
	...
	elseif ( is_singular( 'post' ) ) {
		$sidebar = maghoot_get_mod( 'sidebar_posts' );
	}
	...
	elseif ( is_page() ) {
		if ( hybridextend_is_404() )
			...
			$sidebar = maghoot_get_mod( 'sidebar_pages' );
	}
	// Apply Sidebar Layout for Site
	else {
		$sidebar = maghoot_get_mod( 'sidebar' );
	}

	// Allow for custom manipulation of the layout by child themes
	$sidebar = esc_attr( apply_filters( 'maghoot_main_layout', $sidebar ) );

	// Save the layout for current view
	maghoot_set_current_layout( $sidebar );
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_set_current_layout' ) ):
function maghoot_set_current_layout( $sidebar ) {
	$spans = apply_filters( 'maghoot_main_layout_spans', array(
		'none' => array(
			'content' => 9,
			'sidebar' => 0,
		),
	...
	global $maghoot_theme;
	$maghoot_theme->currentlayout['layout'] = $sidebar;
	if ( isset( $spans[ $sidebar ] ) ) {
		$maghoot_theme->currentlayout['content'] = $spans[ $sidebar ]['content'];
		$maghoot_theme->currentlayout['sidebar'] = $spans[ $sidebar ]['sidebar'];
	} else {
		$maghoot_theme->currentlayout['content'] = $spans['default']['content'];
		$maghoot_theme->currentlayout['sidebar'] = $spans['default']['sidebar'];
	}
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_page_header_attop' ) ):
function maghoot_page_header_attop() {

	$full = array_map( 'trim', explode( ',', maghoot_get_mod( 'page_header_full' ) ) );
	...
	if ( in_array( 'no-sidebar', $full ) ) {
		...
		if ( empty( $sidebar_size ) || hybridextend_is_404() )
			return apply_filters( 'maghoot_page_header_attop', true, 'no-sidebar', $full );
	}

	/* For Posts */
	if ( is_singular( 'post' ) ) {
		if ( in_array( 'posts', $full ) )
			return apply_filters( 'maghoot_page_header_attop', true, 'posts', $full );
		else
			return apply_filters( 'maghoot_page_header_attop', false, 'posts', $full );
	}

	/* For Pages */
	if ( is_page() ) {
		if ( in_array( 'pages', $full ) )
			return apply_filters( 'maghoot_page_header_attop', true, 'pages', $full );
		else
			return apply_filters( 'maghoot_page_header_attop', false, 'pages', $full );
	}

	/* Default */
	if ( in_array( 'default', $full ) )
		return apply_filters( 'maghoot_page_header_attop', true, 'default', $full );
	else
		return apply_filters( 'maghoot_page_header_attop', false, 'default', $full );
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_get_lite_slider' ) ):
function maghoot_get_lite_slider( $type ) {
	$slides = $data = array();
	switch ( $type ) {

		case 'html':
			for ( $i = 1; $i <= 4;  $i++ ) {
				$id = intval( maghoot_get_mod( "wt_html_slide_{$i}-content" ) );
				if ( !empty( $id ) ) {
					$postobj = get_post( $id );
			...
			for ( $i = 1; $i <= 4;  $i++ ) {
				if ( !empty( $data[$i] ) ) {
					...
					// $slides[ $i ]['image'] = maghoot_get_mod( "wt_html_slide_{$i}-image" );
					// $slides[ $i ]['content'] = maghoot_get_mod( "wt_html_slide_{$i}-content" );
					$slides[ $i ]['content_bg'] = maghoot_get_mod( "wt_html_slide_{$i}-content_bg" );
					$slides[ $i ]['button'] = maghoot_get_mod( "wt_html_slide_{$i}-button" );
					$slides[ $i ]['url'] = maghoot_get_mod( "wt_html_slide_{$i}-url" );
					$slides[ $i ]['background']['color'] = maghoot_get_mod( "wt_html_slide_{$i}-background-color" );
					$slides[ $i ]['background']['type'] = maghoot_get_mod( "wt_html_slide_{$i}-background-type" );
					$slides[ $i ]['background']['pattern'] = maghoot_get_mod( "wt_html_slide_{$i}-background-pattern" );
					$slides[ $i ]['background']['image'] = maghoot_get_mod( "wt_html_slide_{$i}-background-image" );
					// $slides[ $i ]['background']['repeat'] = maghoot_get_mod( "wt_html_slide_{$i}-background-repeat" );
					// $slides[ $i ]['background']['position'] = maghoot_get_mod( "wt_html_slide_{$i}-background-position" );
					// $slides[ $i ]['background']['attachment'] = maghoot_get_mod( "wt_html_slide_{$i}-background-attachment" );
				}
			}
			break;
		...
		case 'img':
			for ( $i = 1; $i <= 4;  $i++ ) { 
				$slides[ $i ]['image'] = maghoot_get_mod( "wt_img_slide_{$i}-image" );
				$slides[ $i ]['caption'] = maghoot_get_mod( "wt_img_slide_{$i}-caption" );
				$slides[ $i ]['caption_bg'] = maghoot_get_mod( "wt_img_slide_{$i}-caption_bg" );
				$slides[ $i ]['url'] = maghoot_get_mod( "wt_img_slide_{$i}-url" );
				$slides[ $i ]['button'] = maghoot_get_mod( "wt_img_slide_{$i}-button" );
			}
			break;

	}
	return apply_filters( 'maghoot_get_lite_slider_slides', $slides, $data );
}
endif;
 ...
 * @access public
 */
if ( !function_exists( 'maghoot_jetpack_infinitescroll_render' ) ):
function maghoot_jetpack_infinitescroll_render(){
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_hide_loop_meta_static_frontpage' ) ) :
function maghoot_hide_loop_meta_static_frontpage( $display ) {
	if ( is_front_page() )
		return 'hide';
	else
		return $display;
...
add_filter( 'maghoot_loop_meta_display_title', 'maghoot_hide_loop_meta_static_frontpage' );
 ...
 * @return string
 */
function maghoot_default_archive_type( $archive_type, $context = '' ) {
	$archive_type = maghoot_get_mod( 'archive_type' );
	return $archive_type;
}
add_filter( 'maghoot_default_archive_type', 'maghoot_default_archive_type', 5, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'maghoot_get_skype_button_code' ) ) :
function maghoot_get_skype_button_code( $username ) {
	static $script = false;
	static $id = 1;
	$code = '';
	$action = apply_filters( 'maghoot_skype_button_action', 'call' );
	...
	$code .= '<div id="SkypeButton_Call_' . $username . '_' . $id . '" class="maghoot-skype-call-button">';
	$code .= '<script type="text/javascript">';
	$code .=  'Skype.ui({'
	...
	$code = apply_filters( 'maghoot_get_skype_button_code', $code, $script, $id, $action );
	$script = true;
	$id++;
	return $code;
}
